...
exports.addReputationEvent = onCall({ region: REGION }, async (request) => {
  if (!request.auth) {
    throw new HttpsError("unauthenticated", "Must be signed in");
  }
...
exports.touchReputationActivity = onCall({ region: REGION }, async (request) => {
  if (!request.auth) {
    throw new HttpsError("unauthenticated", "Must be signed in");
  }
  const callerDocId = deriveUserDocId(request.auth.token.email);
...
exports.toggleVote = onCall({ region: REGION }, async (request) => {
...
exports.submitFlag = onCall({ region: REGION }, async (request) => {
  const callerDocId = requireHydAuth(request);
...
exports.submitVerification = onCall({ region: REGION }, async (request) => {
  const callerDocId = requireHydAuth(request);
...
exports.acceptCorrection = onCall({ region: REGION }, async (request) => {
  const callerDocId = requireHydAuth(request);
