...
# Written into campus metadata on every upload, so the live data records which
# code produced it. The Goa column bug reached production because is_course_row
# was committed 2026-07-25 while the deployed revision was still 2026-07-23 —
# a four-day gap that nothing in Firestore could reveal. parser_version.py is
# generated by scripts/predeploy-python.sh and is NOT committed.
try:
    from parser_version import PARSER_VERSION
except ImportError:
...
def parse_lpu(lpu_str):
    """Parse the Goa booklet's "L P U" credit cell.

    Three numbers is the full L/P/U. Two is a practical-only course printing just
    P and U (CHE F312 "3 3", PHY F214 "2 2" — both have P sections and no L).
    One is a bare unit count: projects and theses ("3", "5", and BITS C799T's
    "40"). A few rows carry a footnote asterisk ("3*"), which is not a digit.

    Blank is real too, and not recoverable: the new U-series first-year courses
    print only CREDIT HOUR, so their units are absent from the booklet entirely.
    """
    parts = [d for d in (re.sub(r"\D", "", p) for p in lpu_str.split()) if d]
    if len(parts) >= 3:
        return {"L": _safe_int(parts[0]), "P": _safe_int(parts[1]), "U": _safe_int(parts[2])}
    if len(parts) == 2:
        return {"L": 0, "P": _safe_int(parts[0]), "U": _safe_int(parts[1])}
    if len(parts) == 1:
...
def goa_code_col(data):
    """Which column holds the course code — 0 or 1.

    The 2025-26 booklet led with a COM CODE column, putting the code at 1. The
    2026-27 one dropped COM CODE and added a (blank) CREDIT HOUR column at 3, so
    the code/title/L-P-U block slid left to 0/1/2 while STAT, SEC, INSTRUCTOR and
    everything after stayed exactly where they were. Hardcoding 1 read the title
    as the code and the L-P-U string as the title, which is what put 274
    title-keyed documents into campuses/goa on 2026-07-29.
    """
    return max((0, 1), key=lambda c: sum(
        1 for row in data if COURSE_CODE_RE.match(get_cell_str(row, c))))


def parse_goa_course_group(data, start_row, code_col):
    main_row = data[start_row]
    course_no = get_cell_str(main_row, code_col)
    course_title = get_cell_str(main_row, code_col + 1)
    lpu_str = get_cell_str(main_row, code_col + 2)
    ...
    while current_row < len(data):
        r = data[current_row]
        next_cn = get_cell_str(r, code_col)
        ...
        # A newline here is pdfplumber reporting a wrapped cell, not a separator:
        # "RAVIPRASAD ADURI/ Rajesh\nMehrotra" is two people, not three.
        instructor_str = re.sub(r"\s+", " ", instructor_str)
        instructors = [n.strip() for n in re.split(r"[,/]+", instructor_str) if n.strip()] if instructor_str else []
...
def parse_timetable_rows_goa(data):
    ...
    while current_row < len(data):
        ...
        if is_empty_row(row):
            current_row += 1
            continue
        course_no = get_cell_str(row, code_col)
        if course_no:
            result = parse_goa_course_group(data, current_row, code_col)
            if result:
                courses.append(result["course"])
                current_row = result["nextRow"]
            else:
...
# "L P U" credit cell read as the course title. This is exactly what the Goa
# 2026-07-29 upload wrote for all 274 of its documents.
CREDITS_LIKE_TITLE = re.compile(r"^\d[\d\s*]*$")

# Ceilings, not targets. Measured 2026-07-29 across all three real booklets,
# parsed with the correct page range:
#
#                     goa     hyd     pilani
#   zero credits      3.7%    0.7%    0.0%
#   no schedule      24.4%   22.7%    0.2%
#
# The no-schedule figures are legitimately high — projects, theses and
# independent studies carry no timetabled hours. The Goa column-offset parse
# scored 100% zero-credit, so MAX_ZERO_CREDIT_SHARE alone would have refused it.
MAX_ZERO_CREDIT_SHARE = 0.40
MAX_NO_SCHEDULE_SHARE = 0.60


class CourseSanityError(Exception):
    pass


def sanity_problems(courses):
    """Structural problems that mean the columns were misread, not the data.

    The failure this exists for is not a crash or an empty parse — those are
    obvious. It is a parse that stays the right SHAPE while every field slides
    one column sideways, which is what happened to Goa: 377 rows, every section,
    room, schedule and exam correct, and the code, title and credits each read
    from the neighbouring column. Row counts cannot see that. Field content can.
    """
    problems = []
    n = len(courses)
    if n == 0:
    ...
    def sample(codes):
        # sanitize because the offending "code" is often a wrapped cell, and a
        # newline in the middle of the refusal message renders as a broken toast.
        shown = ", ".join(sanitize(c) for c in codes[:5])
        return shown + (f" (+{len(codes) - 5} more)" if len(codes) > 5 else "")

    # Each of these is zero on all three real booklets, so one occurrence is a
    # defect rather than a threshold to tune.
    credit_titles = [c["courseCode"] for c in courses
                     if CREDITS_LIKE_TITLE.match(sanitize(c.get("courseTitle", "")))]
    if credit_titles:
        problems.append(
            f"{len(credit_titles)} courses have a credit string as their title "
            f"({sample(credit_titles)}) — the code/title/credit columns are "
            f"almost certainly off by one")

    code_titles = [c["courseCode"] for c in courses
                   if COURSE_CODE_RE.match(sanitize(c.get("courseTitle", "")))]
    if code_titles:
        problems.append(
            f"{len(code_titles)} courses have a course code as their title "
            f"({sample(code_titles)}) — code and title look swapped")

    same = [c["courseCode"] for c in courses
            if sanitize(c.get("courseTitle", "")) == sanitize(c.get("courseCode", ""))]
    if same:
        problems.append(f"{len(same)} courses have the title equal to the code ({sample(same)})")

    zero_credit = sum(1 for c in courses if not c.get("totalCredits"))
    if zero_credit / n > MAX_ZERO_CREDIT_SHARE:
        problems.append(
            f"{zero_credit}/{n} courses ({zero_credit / n:.0%}) have zero credits, "
            f"over the {MAX_ZERO_CREDIT_SHARE:.0%} ceiling — the credit column may have moved")

    no_schedule = sum(
        1 for c in courses if not any(s.get("schedule") for s in c.get("sections", [])))
    if no_schedule / n > MAX_NO_SCHEDULE_SHARE:
        problems.append(
            f"{no_schedule}/{n} courses ({no_schedule / n:.0%}) have no schedule, "
            f"over the {MAX_NO_SCHEDULE_SHARE:.0%} ceiling — the days/hours column may have moved")

    return problems


def _guard_course_sanity(courses, force=False):
    problems = sanity_problems(courses)
    if not problems:
        return
    message = "refusing the upload: " + "; ".join(problems)
    if not force:
        raise CourseSanityError(message)
...
def upload_courses_to_firestore(courses, campus_code, clear_first=True, force=False):
    ...
    # Content check before the count check: a column offset keeps the count
    # perfectly healthy, so the count guard alone never sees it.
    _guard_course_sanity(courses, force=force)
    ...
    return (
        ...
        sorted(
            (
                ...
                sanitize(s.get("room", "")),
                tuple(sorted(
                    (tuple(e.get("days", [])), tuple(e.get("hours", [])))
...
def build_timetable_preview(courses, campus_code):
    """What this upload WOULD change, computed without changing anything.

    A row count cannot distinguish "the registrar moved two labs" from "the
    columns shifted and all 377 rows are subtly wrong" — both come back 377.
    This lists the actual documents that differ, so the person clicking upload
    is looking at the change rather than at a total.
    """
    campus_id = CAMPUS_IDS.get(campus_code.lower(), "hyderabad")
    ref = get_db().collection(f"campuses/{campus_id}/timetable")

    incoming = {course_code_to_doc_id(c["courseCode"]): c for c in dedupe_by_doc_id(courses)}
    live = {d.id: (d.to_dict() or {}) for d in ref.get()}

    changed = []
    for doc_id in incoming.keys() & live.keys():
    ...
    added = sorted(incoming.keys() - live.keys())
    removed = sorted(live.keys() - incoming.keys())
    changed.sort()
    ...
    return {
        ...
        "addedSample": added[:PREVIEW_SAMPLE],
        "removedSample": removed[:PREVIEW_SAMPLE],
        "changedSample": changed[:PREVIEW_SAMPLE],
        # Advisory here, fatal at write time — the preview's job is to show the
        # admin why the upload is about to be refused, before they trigger it.
        "problems": sanity_problems(list(incoming.values())),
...
def upload_timetable(req: https_fn.CallableRequest):
    ...
    # trimming its offering); without it the guards below refuse and explain.
    try:
        upload_courses_to_firestore(
            courses, campus_code, clear_first=True, force=bool(req.data.get("force")))
    except (CourseCountDropError, CourseSanityError) as e:
...
def upload_exam_seating(req: https_fn.CallableRequest):
    ...
    from datetime import datetime
    db.document(f"admin_metadata/exam_seating_{campus_id}").set({
        "lastUpdated": datetime.utcnow().isoformat() + "Z",
