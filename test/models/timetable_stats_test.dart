...
void main() {
  group('TimetableStats.allExams', () {
    test('collects every exam, sorted by date then session', () {
      ...
      final stats = TimetableStats.fromTimetable(makeTimetable(courses: courses));
    ...
    test('no exams yields an empty timeline', () {
      final stats = TimetableStats.fromTimetable(
          makeTimetable(courses: [makeCourse(courseCode: 'CS F111')]));
  ...
  group('week shape', () {
    ...
    test('an 8 AM first class marks the day as an early start', () {
      final stats = TimetableStats.fromTimetable(makeTimetable(courses: awkwardWeek()));
    ...
    test('credits are compared against the semester cap', () {
      final light = TimetableStats.fromTimetable(makeTimetable(courses: awkwardWeek()));
      expect(light.totalCredits, 12);
      expect(light.isOverCreditCap, isFalse);

      final heavy = TimetableStats.fromTimetable(makeTimetable(courses: [
        ...awkwardWeek(),
        makeCourse(
          courseCode: 'PHY F211',
          lectureCredits: 12,
